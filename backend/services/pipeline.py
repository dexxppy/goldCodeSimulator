...
from backend.services.ber_detector import calculate_ber, get_error_count
from backend.utils.converting_utils import string_of_bits_to_list
from backend.services.gold_generator import lfsrs_generator, gold_code_generator
from backend.services.Coder import Coder
from backend.services.error_generator import get_error_rate_data, simulate_errors, generate_error_rate
...
def run_transmission_simulation(coded_sentence: List[int], error_rate_lower_limit: int, error_rate_upper_limit: int):
    ...
    decoded_sentence_in_str = decode_data["result_in_str"]
    ...
    return {
        ...
        "decoded_sentence_in_str": decoded_sentence_in_str, "decoded_sentence_in_list": decoded_sentence_in_bit_list,
    }
...
def run_full_simulation(runs: int, sent_to_transmit: str, error_rate_id: int, n=None, seed1=None, seed2=None):
    ...
    for i, run in enumerate(transmission_data):
        print("decoded sent " + str(i))
        print(run["decoded_sentence_in_str"])


    print(ber_rate)


run_full_simulation(runs=20, sent_to_transmit="dupa 538 huj", error_rate_id=3, n=5)
# todo: dostosowac zakresy errorow, skonczyc routy i funkcje symulacji